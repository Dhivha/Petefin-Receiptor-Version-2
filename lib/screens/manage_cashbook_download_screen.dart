...
class ManageCashbookDownloadScreen extends StatefulWidget {
  @override
  _ManageCashbookDownloadScreenState createState() => _ManageCashbookDownloadScreenState();
}

class _ManageCashbookDownloadScreenState extends State<ManageCashbookDownloadScreen> with TickerProviderStateMixin {
  ...
  Future<void> _loadRecentDownloads() async {
    ...
    try {
      final downloads = await _authService.getRecentCashbookDownloads(limit: 10);
      setState(() {
  ...
  bool _hasActiveDownloads() {
    return _recentDownloads.any((download) => 
        download.status == DownloadStatus.pending || 
        download.status == DownloadStatus.downloading);
  }

  int get _activeDownloadsCount {
    return _recentDownloads.where((download) => 
        download.status == DownloadStatus.pending || 
        download.status == DownloadStatus.downloading).length;
  }
    ...
    try {
      ...
      if (result.success) {
        ...
        Future.delayed(Duration(seconds: 2), () {
          if (mounted) {
            _showSnackBar('Download is being processed in the background...', 
  ...
  Future<void> _shareFile(CashbookDownload download) async {
    ...
    try {
      await Share.shareXFiles(
        ...
        text: 'Cashbook report for ${download.branchName} for ${download.formattedDate}',
      );
    } catch (e) {
      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: [
            ...
            Tab(text: _activeDownloadsCount > 0 ? 'Recent Downloads ($_activeDownloadsCount active)' : 'Recent Downloads'),
          ],
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        onPressed: _isRequesting ? null : _requestCashbookDownload,
                        icon: _isRequesting
                            ? SizedBox(
                                width: 20,
                                height: 20,
                        ...
                        label: Text(
                          _isRequesting ? 'Requesting Download...' : 'Request Download',
                          style: TextStyle(
                            fontSize: 16,
                            fontWeight: FontWeight.bold,
                            color: Colors.white,
                ...
                child: Column(
                  children: [
                    ...
                    SizedBox(
                      width: double.infinity,
                      child: ElevatedButton.icon(
                        onPressed: _isProcessing ? null : _processQueuedDownloads,
                        icon: _isProcessing
                            ? SizedBox(
                                width: 20,
                                height: 20,
